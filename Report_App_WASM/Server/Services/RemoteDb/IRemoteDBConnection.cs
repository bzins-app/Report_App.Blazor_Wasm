...
public class RemoteDbConnection : IRemoteDbConnection, IDisposable
{
    ...
    public async Task<string> GetAllTablesScript(int activityId)
    {
        ...
        var dbInfo = await _context.ActivityDbConnection.AsNoTracking().Where(a => a.Activity.ActivityId == activityId)
            .FirstOrDefaultAsync();
        ...
        int taskId = 0)
    {
        ...
        do
        {
            var activityName = await _context.Activity.Where(a => a.ActivityId == run.ActivityId)
                .Select(a => a.ActivityName).FirstOrDefaultAsync();
            ...
            try
            {
                ...
                if (connectionInfo.TypeDb == TypeDb.Oracle)
                {
                    ...
                    {
                        FetchSize = connectionInfo.CommandFetchSize, CommandTimeout = connectionInfo.CommandTimeOut,
                    ...
                    if (connectionInfo.UseDbSchema)
                        dbConnector.IntializationQueries.Add(
                ...
                else if (connectionInfo.TypeDb == TypeDb.Db2)
                {
                    ...
                    dbConnector.DbCommand = new OleDbCommand
                        { CommandTimeout = connectionInfo.CommandTimeOut, CommandType = CommandType.Text };
                }
                else
                {
    ...
    private RemoteConnectionParameter CreateConnectionString(ActivityDbConnectionDto? parameter)
    {
        ...
        {
            Schema = parameter.DbSchema, UseDbSchema = parameter.UseDbSchema, TypeDb = parameter.TypeDb,
            CommandFetchSize = parameter.CommandFetchSize, CommandTimeOut = parameter.CommandTimeOut
        };

        if (parameter.TypeDb == TypeDb.Oracle)
        {
            value.ConnnectionString =
                $"User ID={parameter.ConnectionLogin};Password={parameter.Password}; Data Source={parameter.ConnectionPath};";
        }
        else if (parameter.TypeDb == TypeDb.SqlServer)
        {
            ...
            var databaseInfo = "";
            if (parameter.UseDbSchema) databaseInfo = $";Database={parameter.DbSchema}";

            if (parameter.AdAuthentication)
                connectionString = $"server ={parameter.ConnectionPath}{databaseInfo}{windowsAuthentication};";
            else
                connectionString =
                    $"server ={parameter.ConnectionPath}{databaseInfo};User Id={parameter.ConnectionLogin};Password={parameter.Password};";

            if (parameter.IntentReadOnly) connectionString += "applicationintent=readonly;";
        ...
        else if (parameter.TypeDb == TypeDb.Db2)
        {
            ...
            var databaseInfo = "";
            if (parameter.UseDbSchema) databaseInfo = $";Initial Catalog={parameter.DbSchema}";
            value.ConnnectionString =
                $"Provider=DB2OLEDB.1;Data Source={parameter.ConnectionPath}{databaseInfo};User ID={parameter.ConnectionLogin};Password={parameter.Password};";
        }
        else
        {
            parameter.UseDbSchema = true;
            ...
            if (parameter.UseDbSchema) databaseInfo = $";database={parameter.DbSchema}";
            value.ConnnectionString =
                $"server={parameter.ConnectionPath};port={parameter.Port}{databaseInfo};uid={parameter.ConnectionLogin};password={parameter.Password};";
        }
    ...
    private async Task<RemoteConnectionParameter> GetConnectionString(int activityId)
    {
        ...
        var data = await _context.ActivityDbConnection.Where(a => a.Activity.ActivityId == activityId)
            .FirstOrDefaultAsync();
    ...
    private async Task<DataTable> DbToDataTableAsync(DbGenericParameters dbConnector, RemoteDbCommandParameters run,
        DataTable dataTable, CancellationToken cts)
    {
        ...
        {
            dbConnector.DbCommand.CommandText =
                run.QueryToRun +
                Environment.NewLine; //newline to avoid empty feedback when a comment is open at the last line without CR
            dbConnector.DbDataAdapter.SelectCommand = dbConnector.DbCommand;
            if (run.FillDatatableSchema) dbConnector.DbDataAdapter.FillSchema(dataTable, SchemaType.Source);
        ...
        if (cts.IsCancellationRequested) dataTable.Clear();
