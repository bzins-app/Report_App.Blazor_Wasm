    ...
    public class OracleRemoteDb: IDisposable, IRemoteDb
    {
        ...
        private RemoteConnectionParameter CreateConnectionString(ActivityDbConnection dbInfo)
        {
            RemoteConnectionParameter value = new()
            {
                ...
                ConnnectionString = $"User ID={dbInfo.ConnectionLogin};Password={EncryptDecrypt.EncryptDecrypt.DecryptString(dbInfo.Password)}; Data Source={dbInfo.ConnectionPath};"
            };
        ...
        public string GetAllTablesScript(ActivityDbConnection dbInfo)
        {
            ...
            {
                script = dbInfo.UseDbSchema ? $"SELECT table_name FROM all_tables where owner='{dbInfo.DbSchema}' order by 1" : "SELECT table_name FROM all_tables order by 1";
            }
            return script;
        }
        ...
        public string GetAllTablesAndColumnsScript(ActivityDbConnection dbInfo)
        {
            ...
            if (CheckDbType(dbInfo))
            {
                script = dbInfo.UseDbSchema ? $"SELECT Table_name as Table_Name,column_name as Column_Name FROM ALL_TAB_COLUMNS where owner='{dbInfo.DbSchema}' order by 1,2" : "SELECT Table_name as Table_Name,column_name as Column_Name FROM ALL_TAB_COLUMNS order by 1,2";
            }
            return script;
        }
        ...
        public string GetTableColumnInfoScript(ActivityDbConnection dbInfo, string tableName)
        {
            ...
            if (CheckDbType(dbInfo))
            {
                script = dbInfo.UseDbSchema ? $"select column_name as Column_Name from ALL_TAB_COLUMNS where OWNER='{dbInfo.DbSchema}' and table_name='{tableName}'" : $"select column_name as Column_Name from ALL_TAB_COLUMNS where table_name='{tableName}'";
            }
            return script;
        }
