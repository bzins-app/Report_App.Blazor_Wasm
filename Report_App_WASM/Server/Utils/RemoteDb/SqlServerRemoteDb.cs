...
namespace Report_App_WASM.Server.Utils.RemoteDb
{
    public class SqlServerRemoteDb : IDisposable, IRemoteDb
    {
        ...
        public string GetAllTablesScript(ActivityDbConnection dbInfo)
        {
            ...
            {
                script = dbInfo.UseDbSchema ? $"SELECT table_name FROM information_schema.tables where TABLE_CATALOG='{dbInfo.DbSchema}' order by 1" : "SELECT table_name FROM information_schema.tables order by 1";
            }
            return script;
        }
            ...
            using (var bulkCopy = new SqlBulkCopy(conn))
            {
                ...
                if (totalRows > 1000000)
                {
                    ...
                    for (var i = 0; i <= totalRows; i += batchsize)
                        await bulkCopy.WriteToServerAsync(data.AsEnumerable().Distinct().Skip(i).Take(batchsize).CopyToDataTable());
                }
                else
                {
